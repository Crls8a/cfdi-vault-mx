...
| 3 | RabbitMQ, Redis, and ingestion API contract | Queue/Worker, API, QA | Message contract from Sprint 1 | Exchanges, routing keys, retry, DLQ, progress cache, locks, worker heartbeat, and the FastAPI stored-evidence ingestion contract are implemented or documented. |
| 4 | Fake SAT recovery E2E | SAT Integration, API, Queue/Worker, Data, Storage, QA | Sprints 2, 3, and parser baseline | Fake SAT flow downloads packages, extracts XML, registers evidence, queues ingestion, loads PostgreSQL, reconciles, and exposes job status. |
...
| 8 | Release hardening | PM, Docs, QA, Release, all owners | Core flows accepted | Installer, docs, examples, contribution guide, changelog, test matrix, PyPI package plan, public API contract, and first release candidate are ready. |
...
| Sprint 1 | Metadata reconciliation, RabbitMQ/Redis adapters, FastAPI ingestion contract, parser registry, QA integration harness. |
| Sprint 2 + Sprint 3 | Fake SAT E2E orchestration, queued XML ingestion, search query builder, reconciliation checks. |
...
4. RabbitMQ/Redis durable worker behavior and FastAPI ingestion boundary.
5. Fake SAT end-to-end recovery with queued XML loading into PostgreSQL.
6. CLI operator experience.
7. Security and live SAT opt-in.
8. Public API and PyPI release candidate.
