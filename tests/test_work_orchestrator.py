...
def test_unknown_dependency_fails():
    document = source(); document["items"][0]["dependencies"].append("MISSING")
...
def test_dynamic_blocker_is_actionable():
    document = source(); item = document["items"][0]
...
def test_dependency_and_required_gates_block_prompt():
    document = source(); item = document["items"][-1]
...
def test_non_ready_status_blocks_readiness():
    document = source(); item = dict(document["items"][0]); item["status"] = "in_progress"
...
def test_pr_requires_exactly_one_type_label():
    document = source(); item = dict(document["items"][0])
...
def test_dev_target_valid_and_main_warns():
    document = source(); document["items"][0]["target_branch"] = "main"
...
def test_other_target_fails():
    document = source(); document["items"][0]["target_branch"] = "feature/other"
...
def test_prompt_is_safe_and_useful():
    document = source(); prompt = generated_prompt(document["items"][-1], document)
